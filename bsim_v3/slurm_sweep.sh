#!/bin/bash
#SBATCH --job-name=sweep-4-fixed-eps
...
python bsim_v3/run_sim.py run-multiple --num-simulations 10 --out-file-template "$__expdir"/results-${SLURM_ARRAY_JOB_ID:-$SLURM_JOB_ID}-${SLURM_ARRAY_TASK_ID:-0}-$(hostname).parqueta --eps-scaler 1.0
